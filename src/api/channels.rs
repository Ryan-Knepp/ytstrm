...
pub async fn create_channel(
    ...
    if config.channels.iter().any(|c| c.handle == form.handle) {
        return (
    ...
    let new_channel = Channel {
        id: form.handle.clone(), // Use handle as ID
        name: form.name,
...
pub async fn update_channel(
    ...
    if let Some(channel) = config.channels.iter_mut().find(|c| c.id == id) {
        ...
        channel.handle = form.handle;
        channel.max_videos = form.max_videos;
        channel.max_age_days = form.max_age_days;
...
pub async fn load_channel_videos(
    ...
    if let Some(channel) = config.channels.iter().find(|c| c.id == id) {
        match channel.process_new_videos(&config).await {
            Ok(new_videos) => {
                // Return updated channel component
                Html(format!(r#"
                ...
                "#, channel.name, channel.handle, new_videos, channel.id, channel.id)).into_response()
            }
            Err(e) => {
